...
for x in range(0,num_prev_images):
    ...
    delays.append(acumdelay)
    time_slot=str(datetime.datetime.now() - timedelta(minutes = initial_delay) - timedelta(minutes = acumdelay))
    year.append(time_slot[0:4])
    month.append(time_slot[5:7])
    day.append(time_slot[8:10])
    h=int(float(time_slot[11:13]))-2
    ...
    for j in range(0,60):             
        ...
        if exists(url):
            ...
            print( '  .  .  .  ' + str(x) + ' images found at %s/%s/%s %s:%s:%s' %(str(year[x]),str(month[x]),str(day[x]),str(int(float(hour[x]))+2),str(round_minute[x]),str(second[x])))
...
for i in range(0,len(im_array)):
    ...
    img=equalizeImg(color_dict,RGBS)
...
AvTempMap=expand_Values_in_Map(PosX,PosY,AvTemp)
MaxTempMap=expand_Values_in_Map(PosX,PosY,MaxTemp)
MinTempMap=expand_Values_in_Map(PosX,PosY,MinTemp)
HumidityMap=expand_Values_in_Map(PosX,PosY,Humidity)
